...
async function main() {
  ...
  const results = {};       // "group:home:away" -> [hg, ag]
  const warnings = [];
  let applied = 0;
  ...
  for (const m of data.matches || []) {
    ...
    if (!m.score || !Array.isArray(m.score.ft)) continue; // not finished
    const group = gm[1];
    ...
    results[`${fx.group}:${fx.home}:${fx.away}`] = [goals[fx.home], goals[fx.away]];
    applied++;
  }

  writeResults(results, GROUPS, GROUP_LETTERS, FIXTURES);

  console.log(`Applied ${applied} group results.`);
  if (warnings.length) console.warn('Warnings:\n  ' + warnings.join('\n  '));
}

/* Replace the `const PLAYED_RESULTS = { ... };` block in data.js, emitting
   entries in fixture order (group A..L, matchday order) for stable diffs. */
function writeResults(results, GROUPS, GROUP_LETTERS, FIXTURES) {
  const lines = [];
  let lastGroup = null;
  for (const f of FIXTURES) {
    ...
    const [hg, ag] = results[key];
    lines.push(`  '${key}': [${hg}, ${ag}],`);
  }
  ...
  const re = /const PLAYED_RESULTS = \{[\s\S]*?\n\};/;
  if (!re.test(src)) throw new Error('Could not locate PLAYED_RESULTS block in data.js');
  fs.writeFileSync(DATA_PATH, src.replace(re, block));
}
